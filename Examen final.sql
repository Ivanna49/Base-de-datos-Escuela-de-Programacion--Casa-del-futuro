...
/*2. Mostrar número de pokedex, nombre y altura de los Pokémon de tipo "Roca", ordenados por altura de forma ascendente.
Tablas: pokemon, pokemon_tipo, tipo
Campos: numero_pokedex, nombre, altura
*/
SELECT p.numero_pokedex AS numero, 
...
*/
SELECT p.nombre AS nombre_pokemon, 
		t.nombre AS tipo, 
...
SELECT t.nombre AS tipo_pokemon,
		...
		AVG(eb.velocidad) AS velocidad_promedio,
		AVG(precision_mov) AS precision_promedio
FROM pokemon AS p
INNER JOIN estadisticas_base AS eb 
ON p.numero_pokedex = eb.numero_pokedex 
INNER JOIN pokemon_tipo AS pt 
