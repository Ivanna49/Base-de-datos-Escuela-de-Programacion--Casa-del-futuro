...
SELECT SUM(f.total) AS dinero_gastado
FROM facturas f;
...
SELECT a.titulo AS nombre_album, AVG(c.milisegundos) AS duracion_promedio_ms
FROM canciones c, albumes a
WHERE c.id_album = a.id
GROUP BY a.titulo;
...
SELECT MIN(bytes) AS menor_peso_en_bytes
FROM canciones;
...
SELECT SUM(total) AS suma_total_cliente_6
FROM facturas
WHERE id_cliente = 6
HAVING SUM(total) > 45;